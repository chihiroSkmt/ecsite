 /*  */
 /* 「name」「pssword」をフォームに入力。認証成功時には、「 -- ようこそ! [ full_name ] さん」失敗時には「 -- ようこそ! ゲスト さん」 と表示 */
 let login = (event) => {
	...
	$.ajax({
		...
		datatype: 'json',	/* サーバから返されるデータの型を指定。省略した場合は、jQueryがMIMEタイプなどを見ながら自動的に判別 */
		scriptCharset: 'utf-8'
	})
	.then((result) => {
			let user = JSON.parse(result);	/* 文字列を JSON として解析し、文字列によって記述されているJavaScriptの値やオブジェクトを構築 */
